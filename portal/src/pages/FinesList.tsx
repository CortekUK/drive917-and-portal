...
const FinesList = () => {
  ...
  // Calculate counts from ALL fines, not from filteredFines
  const allFines = finesData?.fines || [];
  const pcnFines = allFines.filter(fine => fine.type === 'PCN');
  const speedingFines = allFines.filter(fine => fine.type === 'Speeding');
  const otherFines = allFines.filter(fine => fine.type === 'Other');
